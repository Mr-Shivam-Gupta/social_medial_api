    ...
    public function index()
{
    ...
    foreach ($private_profiles as $private_profile) {
        if ($follower) {
            $private_posts = Post::select('posts.id', 'content', 'media', DB::raw('COUNT(likes.id) as like_count'), DB::raw('COUNT(comments.id) as comment_count'), DB::raw('IF(MAX(likes.user_id = ' . $user_id . '), true, false) as is_liked'))
    ...
    foreach ($public_profiles as $public_profile) {
        $public_posts = Post::select('posts.id', 'content', 'media', DB::raw('COUNT(likes.id) as like_count'), DB::raw('COUNT(comments.id) as comment_count'), DB::raw('IF(MAX(likes.user_id = ' . $user_id . '), true, false) as is_liked'))
            ->leftJoin('likes', 'posts.id', '=', 'likes.post_id')
            ->leftJoin('comments', 'posts.id', '=', 'comments.post_id')
